    ...
    if os.path.exists(MEMORY_FILE):
        try:
            ...
            with open(MEMORY_FILE, 'r', encoding='utf-8') as f:
                ...
                # 加载角色设定（如果存在）
                loaded_role_system = data.get('role_system', None)
                ...
                # 【格式修复】将非标准角色名转换为标准角色名
                # 标准角色：system, user, assistant
                role_mapping = {
                    "YANG": "assistant",  # 将 "YANG" 转换为 "assistant"
                    "yang": "assistant",
                    "assistant": "assistant",
                    "user": "user",
                    "system": "system"
                ...
                for msg in history:
                    if isinstance(msg, dict) and 'role' in msg:
                        ...
                        # 如果角色不在标准角色中，尝试映射或默认为 assistant
                        fixed_role = role_mapping.get(original_role, "assistant" if original_role not in ["user", "system"] else original_role)
                        fixed_msg = msg.copy()
                        fixed_msg['role'] = fixed_role
                ...
                return history, loaded_role_system
...
def roles(role_name):
    role_dict = {
        "威廉": "研发永生药失败身陷镜子中，1935年借助后代的器官复活，象征科学偏执与冷酷",
        "乌鸦先生": "永生后成为半人半兽，暗中保护家族并引导后人复活威廉，兼具赎罪与操纵性",
        "劳拉": "威廉转世的后代，因范德布尔家族诅咒陷入轮回，最终被控制变成实验品",
        "戴尔": "侦探，在调查劳拉之死的时候陷入轮回，逐渐被诅咒异化，成为新的范德布尔家族代理人",
    }
    return role_dict.get(role_name, ["你是一个普通人，没有特殊角色"])
# 【结束对话规则】
# 告诉AI如何识别用户想要结束对话的意图
# Few-Shot Examples：提供具体示例，让模型学习正确的行为
# 在break_message中使用{role_name}引用选用的角色名
...
conversation_history, loaded_role_system = load_memory()

# 【关键步骤2：确定角色设定】
# 如果从记忆文件中加载了角色设定，使用记忆文件中的角色设定
# 否则使用默认的角色设定
if loaded_role_system and conversation_history:
    # 如果 history 中有 system 消息，尝试从中提取角色设定（去掉结束规则部分）
    if conversation_history[0].get('role') == 'system':
        system_content = conversation_history[0].get('content', '')
        # 提取角色设定部分（在结束规则之前）
        if '【结束对话规则' in system_content:
            role_system = system_content.split('【结束对话规则')[0].strip()
        else:
            role_system = loaded_role_system
        print(f"✓ 已从记忆文件加载角色设定: {role_system[:50]}...")
    else:
        role_system = loaded_role_system
        print(f"✓ 已从记忆文件加载角色设定: {role_system[:50]}...")
else:
    # 如果没有记忆文件或记忆文件中没有角色设定，使用默认角色
    role_system = roles("威廉")
    print(f"✓ 使用默认角色设定: {role_system[:50]}...")
...
# 【关键步骤2：初始化系统提示】
# 检查历史记录中是否已有 system 消息
# 如果记忆为空（第一次运行或文件不存在），需要初始化对话历史
# 如果记忆存在但没有 system 消息，添加 system 消息
